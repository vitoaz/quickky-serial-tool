...
4. `SerialManager` 在后台线程接收串口数据，`SerialManagerQt` 将数据放入有界队列；工作 Tab 定时批量写入接收区。TEXT 内容与快捷指令内部以 CRLF 保存，仅将用户主动输入的换行按每串口配置转换后写入串口，不自动追加换行；HEX 保持字节不变。
5. 配置变更、快捷指令和发送历史由 `ConfigManager` 保存到运行目录的 `config.json`；高频发送历史最多每 2 秒持久化一次，并在窗口关闭时强制写入。
...
- `scripts/`：存放版本信息生成、发布许可证、发布包、Gitee Release 和 PyInstaller 裁剪钩子。
- `.gitee`：存放 Gitee 发布仓库标识和 Git 远程地址，不包含访问令牌。
...
- `themes/`：主题配置目录。
- `scripts/`：构建辅助脚本目录。
- `.gitee`：Gitee 发布仓库配置文件。
- `VERSION`：项目当前版本号的唯一来源文件。
- `licenses/`：Qt/PySide6 发布许可证与第三方声明目录。
- `docs/design/`：项目设计文档目录。
- `docs/guides/`：Python 开发和版本发布操作指南目录。
- `tests/`：标准库回归测试目录。
...
@docs/guides/python-development.md
@docs/guides/version-release.md
...
- 涉及 Python 开发或版本发布时，必须先阅读对应的 `docs/guides/` 指南。
- 版本发布遵循 `docs/guides/version-release.md` 的双 SOP：
  - `SOP 1：预发布准备与确认`：基于上一次正式版后的改动手动生成简洁发布说明，并等待用户确认。
  - `SOP 2：提交与多平台 Release`：仅在用户明确要求“提交”后，才可提交、合并、打标签、推送，并创建 GitHub 与 Gitee Release；仅正式发布完成后，按 SOP 自动提交下一 Snapshot 开发版本。
