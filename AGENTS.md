...
QSerial（Quickky Serial Tool）是一个基于 Python 的桌面串口调试工具，当前默认使用 wxPython 图形界面。它用于配置串口参数、收发文本或十六进制数据，并管理快捷指令、发送历史及本地配置。项目面向需要进行串口通信调试的开发和测试人员。本项目采用 Agent 驱动开发方式，Agent 必须以设计文档和当前代码为依据实施改动。

## 核心逻辑

1. 应用入口创建 wxPython 主窗口，并加载本地配置与主题。
2. 主窗口创建工作区和命令面板，工作区可切换单栏或双栏模式并管理多个 Tab。
3. 每个工作 Tab 读取串口、接收和发送设置，通过 `SerialManager` 打开或关闭对应串口。
4. `SerialManager` 在后台线程接收串口数据，工作 Tab 将数据写入接收区；发送时按 TEXT 或 HEX 模式转换后写入串口。
5. 配置变更、快捷指令和发送历史由 `ConfigManager` 保存到根目录的 `config.json`。

## 模块说明

- `src/main/`：提供 wxPython 与 Tkinter 两套应用启动入口，默认运行脚本使用 wxPython 入口。
- `src/pages/`：实现 wxPython 与 Tkinter 的主窗口和设置对话框。
- `src/components/`：实现工作区、串口设置、数据收发、快捷指令及发送历史等界面组件。
- `src/utils/`：提供串口通信、配置持久化、主题、HEX 转换、文件和对话框等通用功能。
