...
2. `ConfigManager` 在启动加载和导入 JSON 时将缺失、类型错误或不合法的字段恢复为默认值（主题仅允许 `light`、`dark`），丢弃未知结构并按历史上限裁剪发送历史；深层或损坏 JSON 按无效配置处理。仅当配置实际变化时，才先写入同目录临时文件并通过原子替换更新运行目录 `config.json`；高频发送历史的写入间隔至少为 2 秒，窗口关闭时强制刷盘。导入或保存写入失败时保留最近成功持久化的配置快照，并在后续保存时重试。
